...
function kserokopiarka(jaJestemPrawdziwy){
    const nieJa = {...jaJestemPrawdziwy};
    return nieJa;
}
...
function sprawdzam(wuda, piwo){

    let keys1 = Object.keys(wuda).length
    let keys2 = Object.keys(piwo).length
...
function inzynieriaOdtylcowa(odtylca){
    return odtylca.split('').reverse().join(''); 
}
...
function pascal(obiekcik){
    for(kluczyk in obiekcik){
        return obiekcik[kluczyk];
    }
}
...
function xNzq(ksionzka){

    //tutaj cinszko bylo musialem wygooglowac jak sie robi

    const bezDupy = []

    const unique = ksionzka.filter(e => {
        const isDuplicate = bezDupy.includes(e.strona)
        if(!isDuplicate){
            bezDupy.push(e.strona)
            return true;
        }
        return false;
    })

    unique.sort((a, b) => a.strona - b.strona)
...
function detektor(ciong, stringi){
    const iloscLiterek = stringi.length;
    const podzielonyCiong = ciong.split('')
    let i = 0;
    let x = 0;
    let count = 0
    for(i; i < podzielonyCiong.length; i++){
        if(arr[i] === stringi.charAt(x)){
            x++;
            if (x === iloscLiterek) {
                count++;
                x = 0;  
...
//wygenerowalem tom tablice sobie z giepete
...
function mors(tajnykot){
    const tabelkaZKotem = tajnykot.split(" ");
    const tabelaZOdsztfrowanymKotem = [];
    
    for(tajneSlowo of tabelkaZKotem){
        if(morseCodeTable.hasOwnProperty(tajneSlowo)){
            tabelaZOdsztfrowanymKotem.push(morseCodeTable[tajneSlowo]);
        }
    }
    return tabelaZOdsztfrowanymKotem.join('').toLocaleLowerCase()
}
